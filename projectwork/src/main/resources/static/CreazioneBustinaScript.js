...
document.getElementById("bustinaForm").addEventListener("submit", async function (event) {
  ...
  try {
    const response = await fetch("/api/bustina/crea", {
      method: "POST",
      headers: {
    ...
    const responseText = await response.text();
     if (response.ok) {
      alert("Bustina creata con successo!");
    } else {
      alert("Errore durante la creazione della bustina: " + responseText);
    }
    } catch (err) {
    alert("Errore durante la creazione della bustina: " + err.message);
  }
});