...
async function loadData() {
    try {
        ...
        try {
            const cardResponse = await fetch(`${baseUrl}/card/brand/yugiho`);
            if (!cardResponse.ok) {
        ...
        try {
            const bustineResponse = await fetch(`${baseUrl}/bustina/brand/yugiho`);
        ...
        try {
            const boxResponse = await fetch(`${baseUrl}/box/brand/yugiho`);
            if (!boxResponse.ok) {
        ...
        try {
            const accessoriResponse = await fetch(`${baseUrl}/accessori/brand/yugiho`);
...
function createCarousel(containerId, data) {
    ...
    data.forEach(item => {
        ...
        img.src = `img/Yu-Gi-Oh/${item.immagine}`;
        img.alt = item.nome;
        /*img.onerror = () => {
