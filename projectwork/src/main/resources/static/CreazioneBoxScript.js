...
document.getElementById("boxForm").addEventListener("submit", async function (event) {
  ...
  try {
    const response = await fetch("/api/box/crea", {
      method: "POST",
      headers: {
    ...
    if (response.ok) {
      alert("Box creato con successo!");
    } else {
      alert("Errore durante la creazione del box: " + responseText);
    }
  } catch (err) {
    alert("Errore durante la creazione del box: " + err.message);
  }
});